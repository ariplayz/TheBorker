# The Borker

Press F5+F8 at the same time to escape the application.
## Puzzle Solutions

### Puzzle 1: Sequence Analysis
**Question:** 1, 1, 2, 3, 5, 8, 13, 21, 34, ...?
- **Logic:** Fibonacci sequence (each number is the sum of the two preceding ones).
- **Answer:** `55`

### Puzzle 2: Binary Decryption
**Question:** Hexadecimal string: 42 49 54 46. Convert to ASCII.
- **Logic:** 
    - 0x42 = B
    - 0x49 = I
    - 0x54 = T
    - 0x46 = F
- **Answer:** `BITF`

### Puzzle 3: Master Override
**Question:** Find the fault in the kernel pointer. Hint: Famous hexadecimal 'DEAD' value.
- **Logic:** The classic magic number used in systems programming to indicate a crash or "dead" memory.
- **Answer:** `0XDEADBEEF`

---
*Note: This application is a simulation created for entertainment purposes. No files are actually encrypted or deleted.*
